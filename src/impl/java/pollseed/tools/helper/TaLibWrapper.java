...
public class TaLibWrapper implements CommandPrinter {
    ...
    public static void main(String[] args) {
        execute(() -> {
            return C.sma(StandardVal.START_IDX, StandardVal.END_IDX, StandardVal.IN_REAL, StandardVal.OPT_IN_TIME_PERIOD, __outBegIdx,
                    __outNBElement, StandardVal.OUT_REAL);
        });
        execute(() -> {
            return C.rsi(StandardVal.START_IDX, StandardVal.END_IDX, StandardVal.IN_REAL, StandardVal.OPT_IN_TIME_PERIOD, __outBegIdx,
                    __outNBElement, StandardVal.OUT_REAL);
        });
        execute(() -> {
            return C.macd(StandardVal.START_IDX, StandardVal.END_IDX, StandardVal.IN_REAL, AdvancedVal.OPT_IN_FAST_PERIOD,
                    AdvancedVal.OPT_IN_SLOW_PERIOD, AdvancedVal.OPT_IN_SIGNAL_PERIOD, __outBegIdx, __outNBElement, AdvancedVal.OUT_MACD,
                    AdvancedVal.OUT_MACD_SIGNAL, AdvancedVal.OUT_MACD_HIST);
        });
        execute(() -> {
            return C.movingAverage(StandardVal.START_IDX, StandardVal.END_IDX, StandardVal.IN_REAL, AdvancedVal.OPT_IN_TIME_PERIOD, MAType.Kama,
                    __outBegIdx, __outNBElement, StandardVal.OUT_REAL);
        });
    }
